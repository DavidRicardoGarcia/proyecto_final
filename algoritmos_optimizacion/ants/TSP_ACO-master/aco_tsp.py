...
class SolveTSPUsingACO:
    ...
    class Ant:
        ...
        def _select_node(self):
            roulette_wheel = 0.0
            #se revisa en la lista de nodos cuales son los siguientes que no estan en la lista de nodos visitados
            unvisited_nodes = [node for node in range(self.num_nodes) if node not in self.tour]
            heuristic_total = 0.0
            #suma la distancia desde el ultimo nodo guardado en el tour hasta los no visitados 
...
if __name__ == '__main__':
    ...
    _colony_size = 20
    _steps = 100
    _nodes = [(random.uniform(-400, 400), random.uniform(-400, 400)) for _ in range(0, 50)]
    ...
    time, dist =acs.run()
    #time1,dist1=elitist.run()
    #time2,dist2=max_min.run()
    s='el algoritmo ACS     :' + repr(dist) 
    print(s)
    #f='el algoritmo elitist :' + repr(dist1) 
    #print(f)
    #g='el algoritmo max_min :' + repr(dist2) 
    #print(g)
    acs.plot()
